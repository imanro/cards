...
/**
 * Extending Kohana_Formo_ORM trait...
 */
...
trait Formo_ORM {
	...
	public function get_form( array $fields, Formo $form = NULL)
	{
		...
		foreach ($fields as $key => $field_name)
		{
			...
			{
				$try_label = method_exists($this, 'label')? TRUE : FALSE;
				...
				$options = [
					...
					'val' => $this->$field_name,
					'driver' => 'input',
					'label' => $try_label? call_user_func_array(array($this, 'label'), array($field_name)) : $field_name,
				];
	...
	protected function _get_base_rules()
	{
		...
		{
			if (empty($data['is_nullable']) || ($data['is_nullable'] !== TRUE AND ! (in_array(Arr::get($data, 'data_type'), ['set', 'tinyint']))))
			{
